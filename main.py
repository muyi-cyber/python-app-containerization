...
from forms import CreatePostForm, RegisterForm, LoginForm, CommentForm,ForgotPassword,ResetPassword
...
class User(UserMixin, db.Model):
    ...
    def verify_reset_token(token,expiration=30):
        s=serializer(app.config['SECRET_KEY'])
        try:
            email=s.loads(token,max_age=30)
        except:
            return None
...
def forgot_password():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash('A reset link has been sent to your mail','success')
            return redirect(url_for('login'))
        else:
